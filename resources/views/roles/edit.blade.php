				...
				<div class="card-body">
					<form class="p-3" action="{{ route('role.update', $role->id) }}" method="post" >
						...
						<div class="form-group">
							...
							<input type="text" id="name" name="name"  class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" required placeholder="Manager" value="{{ $role->name }}">
										...
										<tr>
											...
											<td>
												<div class="switch switch-sm switch-primary switch-status">
													<input type="checkbox" name="permissions[]" data-plugin-ios-switch value="{{ $permission->id }}" @if($role->permissions->contains($permission)) checked @endif />
												</div>
											</td>
										</tr>
									@endforeach
